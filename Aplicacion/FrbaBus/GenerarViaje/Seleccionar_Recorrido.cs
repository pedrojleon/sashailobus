    ...
    public partial class Seleccionar_Recorrido : Form
    {
        ...
        private void listado_recorridos_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 6)
            {
                ...
                String query = "select ID_RECORRIDO, CODIGO_RECORRIDO, ci_o.NOMBRE_CIUDAD, ci_d.NOMBRE_CIUDAD, ts.DESCRIPCION, ts.ID_TIPO_SERVICIO " +
                               "from SASHAILO.Recorrido re " +
                               "join SASHAILO.Ciudad ci_o on ci_o.ID_CIUDAD = re.ID_CIUDAD_ORIGEN " +
                               "join SASHAILO.Ciudad ci_d on ci_d.ID_CIUDAD = re.ID_CIUDAD_DESTINO " +
